...
# Cấu hình
ESP32_CAM_URL = "http://192.168.1.105/stream"  # Vẫn giữ nếu cần, nhưng không dùng nữa
...
def process_stream():
    ...
    # Dùng webcam máy tính, thay vì ESP32-CAM URL
    cap = cv2.VideoCapture(0)

    if not cap.isOpened():
        print("Error: Cannot open webcam. Retrying in 5 seconds...")
        time.sleep(5)
        return
    ...
    while True:
        ret, frame = cap.read()
        if not ret:
            print("Error: Camera disconnected. Retrying in 5 seconds...")
            time.sleep(5)
            continue
